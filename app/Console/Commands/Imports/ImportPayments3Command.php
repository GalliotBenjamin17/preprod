...
class ImportPayments3Command extends Command
{
    protected $signature = 'imports:payments-3';
    ...
    public function handle(): void
    {
        ...
        $collection = (new FastExcel)->import(public_path('imports/payments-3.xlsx'));
        ...
        foreach ($collection as $row) {
            ...
            $model = match ($row['id_user'] == '') {
                true => Organization::where('id', $row['id_orga'])->first(),
                false => User::where('id', $row['id_user'])->first(),
            };

            $project = Project::where('id', $row['id_projet'])->first();

            if (is_null($model)) {
                $this->info("Can't find ".$row['id_user'].' or '.$row['id_orga']);

                continue;
            }

            // Transaction
            DB::transaction(function () use ($tenant, $project, $row, $model, $defaultCreatedBy) {
