...
@php
    ...
    $quickFilterKey = match ($name) {
        'certification.name' => 'certification',
        'methodForm.name' => 'method',
        'state' => 'state',
...
>
    ...
    >
        <span class="fi-ta-header-cell-label text-sm font-semibold text-gray-950 dark:text-white flex items-center gap-1">
            <span>{{ $slot }}</span>

            @if ($quickFilterKey)
                <button
                    type="button"
                    class="fi-ta-header-cell-filter-btn text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300"
                    title="Filtrer cette colonne"
                    aria-label="Filtrer cette colonne"
                    data-fi-funnel="{{ $quickFilterKey }}"
                >
